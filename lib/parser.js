  ...
  */
  _detectDetailStructure(transaction) {
    ...
    if (prefix === '/') {                // assume /XXX/ fields
      const tagRe    = '\\/[0-9A-Z]{2,4}\\/';
      splitRule.test = `^${tagRe}`;
      splitRule.line = `(?=${tagRe})`;
      splitRule.item = '\\/([0-9A-Z]{2,4})\\/(.*)';
    } else if ('>?'.includes(prefix)) {  // assume >DD fields
      splitRule.test = '^\\' + prefix + '\\d{2}';
      splitRule.line = '\\'  + prefix + '(?=\\d{2})';
      splitRule.item = '(\\d{2})(.*)';
    } else return; // known separator not found
    
    if (!(new RegExp(splitRule.test)).test(details)) return; // string must start with tag
    
    const matches = details.split(new RegExp(splitRule.line));
    if (matches.length > 0 && matches[0].length === 0) matches.shift(); // remove empty match at start
    console.log(matches);
    if (matches.length === 0) return; // no matches found - TODO: refactor ???

    const itemRe = new RegExp(splitRule.item);
    const parsedStruc = matches
      .map(m => m.match(itemRe)) // supposed to match groups 1 and 2
      .reduce((struc, m) => Object.assign(struc, { [m[1]]: m[2] }), {});
    return parsedStruc;
  }
}
